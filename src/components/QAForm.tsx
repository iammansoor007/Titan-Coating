    ...
    try {
      try {
        const response = await fetch(
          "https://formsubmit.co/ajax/info@TitanCoatingsUSA.com",
          {
            method: "POST",
            headers: {
            ...
            body: JSON.stringify({
              _subject: `🏠 Titan Coatings Quote Request - ${formData.name}`,
              name: formData.name,
              email: formData.email,
              phone: formData.phone,
              company: formData.company,
      ...
      window.location.href = `mailto:${email}?subject=🖌️ Titan Coatings Quote Request - ${formData.name}&body=${encodeURIComponent(emailContent)}`;
      setShowSuccess(true);
      setFormStep(1);
      setSelectedServices([]);
      setFormData({
