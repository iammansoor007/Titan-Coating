  ...
  ({
    ...
    >
      <div className="relative">
        <span className="text-2xl sm:text-3xl md:text-4xl font-black" style={{ color: "#C8A35A" }}>
          <Counter value={value} suffix={suffix} />
        </span>
        <div className="absolute -bottom-1 sm:-bottom-2 left-0 w-8 sm:w-12 h-0.5 rounded-full" style={{ background: "linear-gradient(90deg, #C8A35A, #9B7740)" }} />
      </div>
      <p className="text-[10px] sm:text-xs font-bold mt-3 leading-tight uppercase tracking-wide" style={{ color: "#B6B7BA77" }}>
        {label}
      </p>
      </motion.div>
    );
...
export default function AboutSection() {
  ...
  return (
    <section
      ...
      aria-label="About Titan Coatings"
    >
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-8 xl:gap-16 items-stretch ">
          ...
          >
            <motion.div
              ...
              style={{ background: "#C8A35A15", borderColor: "#C8A35A40" }}
            >
              <span className="w-2 h-2 rounded-full animate-pulse" style={{ background: "#C8A35A" }} />
              <span className="uppercase tracking-[0.2em] text-xs font-black" style={{ color: "#C8A35A" }}>
                {badge}
              </span>
            </motion.div>
                ...
                >
                  {coreValues.map((value: string) => (
                    <span
                      ...
                      style={{ background: "#C8A35A15", color: "#C8A35A", border: "1px solid #C8A35A33" }}
                    >
                      {value}
                    </span>
                  ))}
                ...
                {buttons.map((button: any, idx: number) =>
                  button.primary ? (
                    <motion.a
                      ...
                      style={{ background: "linear-gradient(135deg, #C8A35A, #9B7740)", color: "#111", boxShadow: "0 8px 32px rgba(200,163,90,0.35)" }}
                    >
                      <span
                        className="
